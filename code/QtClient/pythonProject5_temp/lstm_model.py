...
def read_single_csv(machine_id, bearing_id):
    ...
    # 构建文件夹路径
    folder_path = f"life_vib_example/machine-{machine_id}/bearing-{bearing_id:02}/"
...
#     lstm = load_model()
#     t_ext_tensor, f_ext_tensor = prepare_data(1, 1)  # 机器号，轴承号
#     res = predict(lstm, t_ext_tensor, f_ext_tensor)
#     print(f"Predicted RUL: {res[0][0] * 161} (min)")

def rul(m, b):
    lstm = load_model()
    t_ext_tensor, f_ext_tensor = prepare_data(m, b)  # 机器号，轴承号
    res = predict(lstm, t_ext_tensor, f_ext_tensor)
    # print(f"Predicted RUL: {res[0][0] * 161} (min)")
    return res[0][0] * 161